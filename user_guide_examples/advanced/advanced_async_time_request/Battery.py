...
def async_time_request_activity(requested_time):
    '''To demonstrate how a non-blocking asynchronous time request works,
    this function is called after the non-blocking request and documents
    its activity. This activity is trivial and only exists to show that 
    code is executing after the time request is made but before being 
    granted
    '''

    # Only do this once a day to avoid slowing the example down too much.
    if requested_time % (60*60*24) == 0:
        logger.warning("Sleeping one second for no good reason.")
        time.sleep(1)
...
if __name__ == "__main__":
    ...
    while grantedtime < total_interval:
        ...
        rv = h.helicsFederateRequestTimeAsync (fed, requested_time)
        logger.debug(f"Returned value from async time request: {rv}")
        async_time_request_activity(requested_time)
        grantedtime =  h.helicsFederateRequestTimeComplete(fed)
